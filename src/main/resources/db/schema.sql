/* =====================================================================
   Mentora LMS - Full SQL Server schema with Question Bank and Classroom Q&A

   This script supports both:
   - Creating a new MentoraDB database from scratch.
   - Upgrading the previous Mentora schema if tables already exist.
   - Migrating legacy classroom answers into the Q&A reply history.
...
/* ---------------------------------------------------------------------
   22. classroom_questions
   Students ask questions inside an active classroom.
   answer_content, answered_by and answered_at are retained as summary
   fields for backward compatibility; reply history is stored below.
   --------------------------------------------------------------------- */
IF OBJECT_ID(N'dbo.classroom_questions', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.classroom_questions (
        ...
        status         NVARCHAR(20) NOT NULL
            CONSTRAINT DF_classroom_questions_status DEFAULT (N'OPEN'),
        created_at     DATETIME2 NOT NULL
            CONSTRAINT DF_classroom_questions_created_at DEFAULT (SYSDATETIME()),
        answered_at    DATETIME2 NULL,
        answered_by    INT NULL,
        answer_content NVARCHAR(2000) NULL,
        CONSTRAINT PK_classroom_questions PRIMARY KEY (id),
        ...
        CONSTRAINT CHK_classroom_questions_status
            CHECK (status IN (N'OPEN', N'ANSWERED'))
    );
...
/* ---------------------------------------------------------------------
   23. classroom_question_replies
   official = 1 for replies from the lecturer or an active classroom TA.
   --------------------------------------------------------------------- */
IF OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.classroom_question_replies (
        ...
        official     BIT NOT NULL
            CONSTRAINT DF_classroom_question_replies_official DEFAULT (0),
        created_at   DATETIME2 NOT NULL
            CONSTRAINT DF_classroom_question_replies_created_at DEFAULT (SYSDATETIME()),
        CONSTRAINT PK_classroom_question_replies PRIMARY KEY (id),
...
BEGIN TRY
    ...
    /* Upgrade Q&A reply history created by an earlier implementation. */
    IF OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NOT NULL
       AND COL_LENGTH(N'dbo.classroom_question_replies', N'official') IS NULL
    BEGIN
        EXEC(N'ALTER TABLE dbo.classroom_question_replies
            ADD official BIT NOT NULL
                CONSTRAINT DF_classroom_question_replies_official DEFAULT (0);');
    END;

    /* Promote replies that are certainly official. Existing official values
       are preserved; rerunning the script never demotes a historical reply. */
    IF OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NOT NULL
    BEGIN
        EXEC(N'
            UPDATE r
            ...
            INNER JOIN dbo.classroom_questions q ON q.id = r.question_id
            INNER JOIN dbo.users u ON u.id = r.responder_id
            INNER JOIN dbo.roles roles ON roles.id = u.role_id
            LEFT JOIN dbo.classroom_members cm
                ON cm.classroom_id = q.classroom_id
               AND cm.user_id = r.responder_id
               AND cm.status = N''ACTIVE''
            WHERE r.official = 0
              AND (
                  roles.name = N''LECTURER''
                  OR cm.role_in_class = N''TA''
              );
        ');
    END;

    /* Migrate the legacy single-answer fields once per question. Student
       replies do not prevent the official legacy answer from being copied. */
    IF OBJECT_ID(N'dbo.classroom_questions', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NOT NULL
       AND COL_LENGTH(N'dbo.classroom_questions', N'answer_content') IS NOT NULL
    BEGIN
        EXEC(N'
            INSERT INTO dbo.classroom_question_replies
                (question_id, responder_id, content, official, created_at)
            SELECT
                q.id,
                q.answered_by,
                q.answer_content,
                1,
                COALESCE(q.answered_at, q.created_at, SYSDATETIME())
    ...
    /* Keep the legacy summary columns and status aligned with the newest
       official reply. This block is idempotent. */
    IF OBJECT_ID(N'dbo.classroom_questions', N'U') IS NOT NULL
       AND OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NOT NULL
    BEGIN
        EXEC(N'
            UPDATE q
            SET status = CASE
                    WHEN official_reply.id IS NOT NULL THEN N''ANSWERED''
                    ELSE N''OPEN''
                END,
                answered_by = official_reply.responder_id,
                answered_at = official_reply.created_at,
                answer_content = official_reply.content
            FROM dbo.classroom_questions q
            OUTER APPLY (
                SELECT TOP (1)
                    r.id,
    ...
    /* Create indexes needed by Classroom Q&A queries. */
    IF OBJECT_ID(N'dbo.classroom_questions', N'U') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1 FROM sys.indexes
           WHERE name = N'IX_classroom_questions_classroom_created'
             AND object_id = OBJECT_ID(N'dbo.classroom_questions')
       )
        CREATE INDEX IX_classroom_questions_classroom_created
            ON dbo.classroom_questions(classroom_id, created_at DESC);
    ...
    IF OBJECT_ID(N'dbo.classroom_questions', N'U') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1 FROM sys.indexes
           WHERE name = N'IX_classroom_questions_student'
             AND object_id = OBJECT_ID(N'dbo.classroom_questions')
       )
        CREATE INDEX IX_classroom_questions_student
            ON dbo.classroom_questions(classroom_id, student_id, created_at DESC);
    ...
    IF OBJECT_ID(N'dbo.classroom_question_replies', N'U') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1 FROM sys.indexes
           WHERE name = N'IX_classroom_question_replies_question_created'
             AND object_id = OBJECT_ID(N'dbo.classroom_question_replies')
       )
        CREATE INDEX IX_classroom_question_replies_question_created
            ON dbo.classroom_question_replies(question_id, created_at ASC);
...
PRINT N'>>> MentoraDB schema with Question Bank and Classroom Q&A is ready.';
GO
